...
const tiRecommend = {
  ...
  actions: {
    async queryTiData({ commit, state }) {
      ...
      try {
        if (Array.isArray(result.head) && !result.head.length) {
          commit('saveShallow', {
          ...
          return;
        } else if (result.head.list && !result.head.list.length) {
          commit('saveShallow', {
            head: { list: [], total_cost: '' },
            details: { list: [], total_cost: '' },
            tableLoading: false
    ...
    queryFilterData({ commit, state }, { product_id, channel_id }) {
      ...
      if (product_id) {
        ...
        commit('save', {
          filterData: {
            product_list,
            channel_list: channel_list_filter,
            agent_list: agent_list_filter
          },
          formInline: {
