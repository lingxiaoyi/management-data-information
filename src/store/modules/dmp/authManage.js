...
const tiRecommend = {
  ...
  actions: {
    ...
    async querySelectDataAuthManage({ commit, state }) {
      if (state.selectData.product_list) return;
      commit('save', {
        tableLoading: true
      });
      let result = await request({
        url: URL.oauthAccount.params,
        method: 'get',
      ...
      commit('save', {
        selectData: result,
        tableLoading: false,
        accountList: result.account,
        formInline: {
          product_id: result.product_list[0].product_id,
          channel_id: result.channel_list[0].channel_id,
          agent_id: result.agent_list[0].agent_id,
        },
        oldformInline: {
          product_id: result.product_list[0].product_id,
          channel_id: result.channel_list[0].channel_id,
