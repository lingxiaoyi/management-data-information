<template>
  <div class="main">
    ...
    <h3 class="table">
      汇总媒体:
      <span>{{ head.total_channel }}</span>， 累计消费金额:
      <span>{{ (head.total_cost / 1).toFixed(2) }}</span>元， 激活数据量
      <span>{{ head.total_active }}</span>， 人均激活成本:
      <span>{{ head.per_cost }}</span>元
    </h3>
    <base-table
    ...
    <base-table
      ...
      @tableCellClick="tableCellClick"
      :tableViewHeight="500"
      :defaultSort="{prop: 'cost', order: 'descending'}"
    />
    <div class="block">
      <el-pagination
...
export default {
  ...
  beforeRouteLeave(to, from, next) {
    ...
    if (to.name !== 'tiRecommendDetail' && to.name !== 'tiRecommendAgent') {
      this.save({
  ...
  methods: {
    ...
    ...mapActions('tiRecommend', ['queryTiData', 'querySelectData', 'queryFilterData', 'exportTable']),
    tableCellClick(row, columns) {
      ...
      } else if (this.pagetype === 'base' && prop === 'date' && row.date.indexOf('~') > 0) {
        this.$router.push({
  ...
  watch: {
    ...
    channel_id(val, oldVal) {
      if (val === oldVal) return;
      this.querySelectData().then(() => {
        this.queryFilterData({ channel_id: val });
        this.queryTiData();
      });
    },
    datepicker: {
      handler(val) {
        if (val.rangeDate === null) return;
        this.save(val);
        this.queryTiData();
        if (this.isRangeDate) {
          //区间日期
          let columns1 = cloneDeep(baseShortColumns);
